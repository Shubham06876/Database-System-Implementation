...
public:
	...
	//	Get my file destination
	int GetMyFilDes();
	
	// returns the current length of the file, in pages
	off_t GetLength ();

	//sets the length of the file to curLength-1 in case we get a non-full page for writing
	void SetLength(off_t len);
	...
	//Appends page addMe to end of file
	void AppendPage(Page *addMe);
