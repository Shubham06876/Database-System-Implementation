...
void q6 () {
	...
	Project P;
		Pipe __s_ps (pipesz);
		int keepMe[] = {3, 10};
		int numAttsIn = outAtts;
		int numAttsOut = 2;
	P.Use_n_Pages (buffsz);
	Attribute projAtt[] = {s_nationkey, ps_supplycost};
	Schema proj_sch ("proj_sch", numAttsOut, projAtt);

	GroupBy G;
		Pipe _out (1);
		Function func;
			char *str_sum = "(ps_supplycost)";
			get_cnf (str_sum, &proj_sch, func);
			//cout<<"function: "; func.Print ();
		OrderMaker grp_order;
		get_sort_order("(s_nationkey)", proj_sch, grp_order);
		//cout<<"group order: "; grp_order.Print();
	G.Use_n_Pages (1);

	SF_s.Run (dbf_s, _s, cnf_s, lit_s); // 10k recs qualified
	SF_ps.Run (dbf_ps, _ps, cnf_ps, lit_ps); // 161 recs qualified
	J.Run (_s, _ps, _s_ps, cnf_p_ps, lit_p_ps);
	P.Run (_s_ps, __s_ps, keepMe, numAttsIn, numAttsOut);
	G.Run (__s_ps, _out, grp_order, func);
	...
	cout << " query6 returned sum for " << cnt << " groups\n"; 
}
/*void q6 () {
