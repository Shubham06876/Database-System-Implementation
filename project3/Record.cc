...
//clears contents of this record
void Record::Clear(){
	bits = NULL;
}
...
void Record :: Print (Schema *mySchema, std::ostream &os) {
...
void Record::PrintBits(Schema *mySchema) {

	//int pointer = ((int *) bits)[0];
	//int *myInt = (int *) &(bits[pointer]);
	//cout << *myInt<<endl;
...
bool Record::IsEqual(Record *withme){
	return (strcmp(GetBits(), withme->GetBits()) == 0);
}