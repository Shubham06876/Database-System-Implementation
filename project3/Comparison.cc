...
//TODO - why is delete creating seg faults
OrderMaker :: OrderMaker(OrderMaker *sortorder, CNF *cnf) {
	int soNumAtts = sortorder->GetNumAtts(); cout<<"soNumAtts: "<<soNumAtts<<endl;
	int *soWhichAtts = sortorder->GetWhichAtts();
	Type *soWhichTypes = sortorder->GetWhichTypes();
	int cnfNumAnds = cnf->GetNumAnds(); cout<<"cnfNumAnds: "<<cnfNumAnds<<endl;
	int *cnfOrLens = cnf->GetOrLens();
	numAtts = 0;

	for(int k=0; k<soNumAtts; k++) {
		bool isAdded = false;
		for(int i=0; i<cnfNumAnds; i++) {
			Comparison *cnfOrList = cnf->GetOrList(i); 
			for(int j=0; j<cnfOrLens[i]; j++) {
				if(cnfOrList[j].GetAtt1() == soWhichAtts[k]){
					if(cnfOrList[j].GetOperand2() == Literal){
						if(cnfOrList[j].GetOp() == Equals){
							whichAtts[numAtts] = soWhichAtts[k];
							whichTypes[numAtts] = soWhichTypes[k];
							numAtts++;
							isAdded = true;
						}
					}
				}else if(cnfOrList[j].GetAtt2() == soWhichAtts[k]){
					if(cnfOrList[j].GetOperand1() == Literal){
						if(cnfOrList[j].GetOp() == Equals){
							whichAtts[numAtts] = soWhichAtts[k];
							whichTypes[numAtts] = soWhichTypes[k];
							numAtts++;
		...
		// found an attribute in sorting ordermaker that is not in query cnf, stop building query ordermaker
		if(!isAdded) break;
	}
...
// set numAtts
void OrderMaker:: SetNumAtts(int num){
        cout <<"set numatts called with : " << num << "\n";
        numAtts = num;
}

//set whichAtts
void OrderMaker:: SetWhichAtts(int which[]){
        for(int i=0;i<numAtts ;i++){
                whichAtts[i] = which[i];
        }
}
...
//set whichTypes
void OrderMaker:: SetWhichTypes(Type which[]){
        for(int i=0;i<numAtts ;i++){
                        whichTypes[i] = which[i];
        }
}
...
Comparison* CNF :: GetOrList(int i){
    return orList[i];

}

int Comparison:: GetAtt1(){
    return whichAtt1;
}

int Comparison:: GetAtt2(){
    return whichAtt2;
}
