...
void File::AppendPage(Page *addMe){
	off_t page_no = curLength;
	if (page_no != 0) page_no--;

	AddPage(addMe, page_no);
}
...
//mkostemp - create a unique temporary file
  myFilDes = mkostemp (fName, S_IRUSR | S_IWUSR);
	...
	// see if there was an error
	if (myFilDes < 0) {
		cerr << "BAD!  CreateTemp did not work for " << fName << "\n";
		exit (1);
	}
